...
function _omb_theme_PROMPT_COMMAND {
  ...
  * ) my_ps_host="${_omb_prompt_green}\h${_omb_prompt_normal}";
  ...
  # nice prompt
  case $(id -u) in
  0) PS1="${TITLEBAR}[$my_ps_root][$my_ps_host]$(modern_scm_prompt)$(__my_rvm_ruby_version)[${_omb_prompt_teal}$THEME_PROMPT_WORKDIR${_omb_prompt_normal}]$(is_vim_shell)
$ "
     ;;
  *) PS1="${TITLEBAR}[$my_ps_user][$my_ps_host]$(modern_scm_prompt)$(__my_rvm_ruby_version)$(__my_venv_prompt)[${_omb_prompt_teal}$THEME_PROMPT_WORKDIR${_omb_prompt_normal}]$(is_vim_shell)
$ "
     ;;
  esac
}
