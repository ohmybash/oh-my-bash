...
function _omb_theme_vscode_initialize {
    local userpart='`export XIT=$? \
        && [ ! -z "${GITHUB_USER}" ] && _omb_util_put "\[\033[0;32m\]@${GITHUB_USER} " || _omb_util_put "\[\033[0;32m\]\u " \
        && [ "$XIT" -ne "0" ] && _omb_util_put "\[\033[1;31m\]➜" || _omb_util_put "\[\033[0m\]➜"`'
    local gitbranch='`\
