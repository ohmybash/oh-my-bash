...
function _omb_completion_ssh {
  ...
  if ((${#config_files[@]} != 0)); then
    COMPREPLY+=($(compgen -W "$(awk '/^[Hh][Oo][Ss][Tt] / { current=""; for (i=2; i<=NF; i++) { hosts[$i]=1; current=$i; }}/^[ \t]*[Hh][Oo][Ss][Tt][Nn][Aa][Mm][Ee][ \t]+/ && current { if (!($2 in hosts)) hosts[$2]=1;}END { for (host in hosts) print host;}' "${config_files[@]}")" "${options[@]}"))
  fi
